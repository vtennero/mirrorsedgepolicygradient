...
\begin{itemize}
    ...
    \item Stochastic reward shaping (40\% style frequency): 10.21\% roll usage, +78.52 final reward
    \item Roll usage increased 14.8× (0.69\% to 10.21\%) compared to baseline
    \item Final performance: +78.52 average reward, 360.15 units mean distance traveled (range 26.09--582.92 units)
\end{itemize}
...
Result: 16\% reward improvement (+67.90→+78.52), strategic roll usage (10.21\%).
...
\textbf{Final Performance Metrics (2M steps):} Reward: +78.52 (0.3\% improvement over previous best, 16\% over roll system v1). Training progressed monotonically: +39.95 at 500k → +66.25 at 1M → +77.75 at 1.5M → +78.52 at 2M steps (97\% improvement from 500k). Policy Loss: 0.0240 (stable), Value Loss: 1.027, Entropy: 0.600 (high exploration maintained). Hyperparameter decay: Learning rate $3.0 \times 10^{-4}$ (linear decay), Beta 0.1 (linear decay), Epsilon 0.2 (linear decay).
...
Mean episode length 43.91 seconds (range 4.54--66.46), mean distance 360.15 units (range 26.09--582.92). All 100 logged episodes (sampled every 20,000 steps) reached success. Low-distance episodes ($<50$ units) indicate early exploration; high-distance episodes ($>500$ units) indicate successful target reach with efficient action usage.
...
\bottomrule
\end{tabular}
\caption{Action distribution statistics (final step: 2M)}
\end{table}

\textbf{Agent Behavior Analysis:}
\begin{itemize}
    \item \textbf{Roll Usage:} 10.21\% of actions (vs 0.69\% in previous run with 15\% style frequency)
    \item \textbf{Roll Improvement:} 14.8× increase over previous run (0.69\% → 10.21\%)
    \item \textbf{Strategic Roll Usage:} Rolls used at 10.21\% despite high cost (60 stamina), indicating learned strategic value
\end{itemize}
...
The stochastic reward shaping (40\% style frequency) increased roll usage from 0.69\% to 10.21\% (14.8× increase) and final reward from +67.90 to +78.52 (16\% improvement) compared to baseline configurations. Figure 4 shows 11 baseline runs exploring learning rates ($3 \times 10^{-4}$ to $5 \times 10^{-4}$), beta values (0.05 to 0.2), style frequencies (0\%, 10\%, 15\%), and roll costs (60--150 stamina); all underperformed the final 40\% dual-reward configuration.

\textbf{Key Behavioral Changes:}
\begin{enumerate}
    \item \textbf{Roll Integration:} Agent learned to use rolls strategically (10.21\% usage) despite high stamina cost (60 per roll)
    \item \textbf{Stamina Management:} Agent balances sprint (13.59\%) and roll (10.21\%) usage, maintaining stamina for critical actions
    \item \textbf{Movement Diversity:} Primary movement is jog (66.97\%), with strategic use of sprint and roll for speed and style
\end{enumerate}

\textbf{Limitations:} While roll usage increased 14.8×, we lack human validation that the learned roll timing is aesthetically pleasing. The agent may execute rolls at mechanically optimal but visually awkward moments. Future work should compare roll distribution between style (40\%) and non-style (60\%) episodes to validate the episodic shaping mechanism.
...
\begin{itemize}
    ...
    \item \textbf{Policy Convergence:} Policy loss stabilized at 0.0240, indicating converged policy
    \item \textbf{Value Estimation:} Value loss at 1.027 reflects reasonable estimation error
    \item \textbf{Exploration:} Policy entropy maintained at 0.600, indicating continued exploration even at convergence
\end{itemize}

\textbf{Learning Rate Decay:}
The linear decay schedule successfully shifted from exploration to exploitation:
\begin{itemize}
    \item Initial learning rate: $3.0 \times 10^{-4}$ (linear decay)
    \item Beta: 0.1 (linear decay)
    \item Epsilon: 0.2 (linear decay)
\end{itemize}
...
Target positioned at $lastPlatformEndX + 5.0$ units (beyond final platform). Success condition: $|agent.x - target.x| < 2.0$ units (X-axis only). For a typical successful episode ($\sim$500 units progress): progress provides $\sim$50.0 reward, target reach +10.0, with remaining from grounded/stamina/roll bonuses contributing to final reward of +78.52.
